...
mysqli_query($conexion, $sql) or die("Problemas en el insert del Integrante: ".mysqli_error($conexion));
mysqli_close($conexion);

$conexion = mysqli_connect("localhost", "root", "usuario.1234", "proyectophp") or die("Problemas de Conexión");
$sql = "INSERT INTO mision(nombre,vehiculo,fecha_inicio,ID_tripulante) SELECT ('$mision','$nave','$fecha',ID_tripulante) FROM integrantes WHERE nom_ape LIKE $comandante";
mysqli_query($conexion, $sql) or die("Problemas en el insert de la Mision: ".mysqli_error($conexion));
mysqli_close($conexion);
